...
Si tengo documento que arranca en linea, por ejemplo, entonces indentar 2 (Linea 1 (texto "a"))
debería devolver Linea 1 (Texto "a" Vacio)

Por lo tanto, para nuestra primera solucion consideramos para resolver un acercamiento usado en
problemas recursivos, que se aplica en backtracking, divide and conquer, o cualquier tipo de problema
con algoritmos recursivos, y era el usar a indentar como wrapper de una función auxiliar que mantiene
...
No hace falta leer lo que sigue, pero aqui dejamos algunos casos de test que hicimos para la
vesion legacy de indentar. (No estan escritos para ser ejecutados directamente por HUnit)

indentar 3 (linea <+> texto "a" <+> linea <+> texto "b") deberia devolver lo mismo que
linea <+> texto "a" <+> Linea 3 (texto "b")

indentar 4 (texto "a" <+> linea <+> texto "b" <+> linea <+> texto "c") deberia devolver lo mismo que
texto "a" <+> Linea 4 (texto "b" <+> Linea 4 (texto "c"))

indentar 2 (Linea 1 (Linea 3 (texto "a"))) deberia ser igual que Linea 1 (Linea 5 (texto "a")) 